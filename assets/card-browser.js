...
(function () {
  ...
  function open() {
    if (!cardsDb) {
      var listEl = document.getElementById('cbCardList');
      if (listEl) listEl.innerHTML = '<div class="cb-loading">Chargement de la biblioth\u00e8que\u2026</div>';
      loadCardsDb().then(function () {
        ...
        var el = document.getElementById('cbCardList');
        if (el) el.innerHTML = '<div class="cb-empty">Erreur de chargement : ' + escHtml(e.message) + '</div>';
      });
    } else {
      syncUserCards();
      applyFilters();
