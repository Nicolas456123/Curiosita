...
(function () {
  ...
  function initTabs() {
    // Scoped tab switching — each [data-tab-group] container manages its own tabs
    document.querySelectorAll('[data-tab-group]').forEach(container => {
      const tabs = container.querySelectorAll('.sr-tab');
      ...
      tabs.forEach(tab => {
        tab.addEventListener('click', () => {
          ...
          parent.querySelectorAll(':scope > .sr-panel').forEach(p => p.classList.remove('active'));
          const panel = parent.querySelector('#panel-' + target);
  ...
  function initKeyboard() {
    document.addEventListener('keydown', (e) => {
      // Only when Reviser panel is active
      const reviewPanel = $('#panel-reviser');
  ...
  // Expose for main panel switching
  window.initReviewPanel = function() {
    updateDashboard();
    buildFilterOptions();
    initReview();
  };
